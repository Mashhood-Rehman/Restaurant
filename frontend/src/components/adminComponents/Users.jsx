...
const Users = () => {
  ...
  const showMessagePanel = Boolean(userId)
  ...
  const [createUser, { isLoading: isCreatingUser }] = useCreateUsersMutation()
  const { data: usersData, error, refetch } = useGetAllUsersQuery()

  const selectedUser = usersData?.getUsers?.find((u) => String(u.id) === String(userId))
